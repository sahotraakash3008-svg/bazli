...
async function startServer() {
  ...
  // ==========================================
  // REAL-TIME GPS DELIVERY TRACKING RADAR (Item 3)
  // ==========================================
  const activeRiderGpsLocations: Record<string, {
    orderId: string;
    partnerId: string;
    partnerName: string;
    lat: number;
  ...
  // Broadcast real mobile GPS from Rider's device
  app.post('/api/delivery/broadcast-location', (req, res) => {
    try {
      const { orderId, partnerId, partnerName, lat, lng, speed, heading, accuracy, status = 'Delivering' } = req.body;
      if (!orderId || typeof lat !== 'number' || typeof lng !== 'number') {
        return res.status(400).json({ success: false, message: 'Missing orderId, lat, or lng' });
      }

      const locPayload = {
        orderId,
        partnerId: partnerId || 'delivery-partner-express',
        partnerName: partnerName || 'Bazli Express Rider',
        lat,
        lng,
        speed: speed || 28,
        heading: heading || 0,
      ...
      activeRiderGpsLocations[orderId] = locPayload;

      // Also update delivery partner position
      const partner = deliveryPartners.find(p => p.id === partnerId);
      if (partner) {
        (partner as any).currentLocation = { lat, lng };
      }

      // Update order if found
      const order = orders.find(o => o.id === orderId);
      if (order && (order as any).riderLocation) {
        (order as any).riderLocation = { lat, lng, updatedAt: Date.now() };
      }

      res.json({
        success: true,
        message: 'GPS coordinates broadcasted live to customer map',
        data: locPayload
      });
    } catch (err: any) {
      res.status(500).json({ success: false, message: 'Failed to broadcast GPS', error: err?.message });
    }
  });

  // Customer fetches live GPS coordinates of their delivery partner
  app.get('/api/delivery/live-location/:orderId', (req, res) => {
    const { orderId } = req.params;
    const location = activeRiderGpsLocations[orderId] || null;
    res.json({
      success: true,
      orderId,
      location,
      hasLiveGps: Boolean(location && (Date.now() - location.updatedAt < 60000))
    });
  });

  // ==========================================
  // REAL-TIME SMS & WHATSAPP ORDER ALERTS (Item 2)
  // Triggered on Order Confirmed, Order Packed, Out for Delivery, and Delivered
  // ==========================================
  app.post('/api/notifications/order-status-alert', async (req, res) => {
    try {
      const { orderId, newStatus, customerPhone, customerName, riderName, deliveryOtp, trackingUrl } = req.body;
      const order = orders.find(o => o.id === orderId);

      const targetPhone = customerPhone || order?.customerPhone || '9871618126';
      const cleanPhone = String(targetPhone).replace(/\D/g, '').replace(/^0+/, '');
      const formattedPhone = cleanPhone.length === 10 ? `91${cleanPhone}` : cleanPhone;
      const name = customerName || order?.customerName || 'Valued Customer';
      const otp = deliveryOtp || order?.deliveryOtp || '4821';
      const rider = riderName || order?.deliveryPartnerName || 'Bazli Express Rider';
      const amount = order?.finalAmount || 0;
      const liveLink = trackingUrl || `${req.headers.origin || 'https://bazli.in'}/#track=${orderId || 'live'}`;
      ...
      switch (newStatus) {
        case 'Confirmed':
          alertMessage = `🛍️ *BAZLI ORDER #${orderId} CONFIRMED!* ⚡\n\nHi ${name},\nYour order of ₹${amount} is confirmed and sent to store for priority packing.\n\n🔐 *Doorstep OTP:* *${otp}*\n🛵 *Assigned Rider:* ${rider}\n\n📍 *Track Live GPS:* ${liveLink}\n\nDelivering in 10-15 minutes! 🚀`;
          shortSms = `Bazli Order #${orderId} Confirmed! Total: Rs.${amount}. Doorstep OTP: ${otp}. Rider: ${rider}. Track: ${liveLink}`;
          break;
        case 'Packed':
        case 'Preparing':
          alertMessage = `📦 *ORDER #${orderId} PACKED & SEALED!*\n\nHi ${name},\nYour fresh items have been hygienically packed and handed to rider ${rider}.\n\n🔐 *Delivery OTP:* *${otp}*\n📍 *Track Rider:* ${liveLink}`;
          shortSms = `Bazli Order #${orderId} packed fresh and handed to rider ${rider}. Doorstep OTP: ${otp}. Track: ${liveLink}`;
          break;
        case 'Out for Delivery':
        case 'Picked Up':
          alertMessage = `🛵 *RIDER IS ON THE WAY WITH ORDER #${orderId}!* ⚡\n\nHi ${name},\n${rider} has picked up your package and is cruising towards your doorstep.\n\n🔐 *Share this OTP with rider at delivery:* *${otp}*\n\n📍 *Live Satellite GPS:* ${liveLink}\nETA: Under 10 minutes!`;
          shortSms = `Rider ${rider} is OUT FOR DELIVERY with Bazli Order #${orderId}! Share OTP ${otp} at doorstep. Live GPS: ${liveLink}`;
          break;
        case 'Delivered':
          alertMessage = `🎉 *ORDER #${orderId} DELIVERED!* ✅\n\nHi ${name},\nYour order has been successfully delivered by ${rider}. Enjoy your fresh items!\n\nRate your delivery & earn Bazli Gold Coins: ${liveLink}\nThank you for choosing Bazli! ❤️`;
          shortSms = `Bazli Order #${orderId} delivered successfully! Enjoy your fresh items. Rate your delivery: ${liveLink}`;
          break;
        default:
          alertMessage = `ℹ️ *BAZLI ORDER #${orderId} UPDATE:* Status is now "${newStatus}". Track: ${liveLink}`;
          shortSms = `Bazli Order #${orderId} update: ${newStatus}. Track: ${liveLink}`;
      }
      ...
      if (fast2smsKey && cleanPhone.length === 10) {
        try {
          ...
          const smsData = await smsRes.json();
          if (smsData && smsData.return) smsSent = true;
        } catch (e) {
          console.warn('Fast2SMS dispatch alert note:', e);
        }
      }
      ...
      if (twilioSid && twilioAuth && twilioFrom) {
        try {
          ...
          if (twilioRes.ok) twilioSent = true;
        } catch (twErr) {
          console.warn('Twilio dispatch alert note:', twErr);
        }
      }

      // WhatsApp Cloud API integration
      let waCloudSent = false;
      const whatsappKey = process.env.WHATSAPP_API_KEY;
      const whatsappPhoneId = process.env.WHATSAPP_PHONE_ID;
      if (whatsappKey && whatsappPhoneId) {
        try {
          ...
          const waData = await waRes.json();
          if (waData && waData.messages) waCloudSent = true;
        } catch (wErr) {
          console.warn('WhatsApp Cloud API alert note:', wErr);
        }
      }

      // Generate clickable instant WhatsApp URL
      const waClickToChatUrl = `https://api.whatsapp.com/send?phone=${formattedPhone}&text=${encodeURIComponent(alertMessage)}`;
      ...
      auditLogs.unshift({
        ...
        timestamp: new Date().toISOString(),
        actorRole: 'system',
        actorName: 'Order Alert Dispatcher',
        action: 'ORDER_ALERT_' + (newStatus || 'UPDATE').toUpperCase().replace(/\s+/g, '_'),
        details: `Alert sent for #${orderId} (${newStatus}) to +${formattedPhone}. SMS: ${smsSent}, Twilio: ${twilioSent}, WhatsApp: ${waCloudSent}`
      });
      ...
      res.json({
        ...
        channels: {
          fast2sms: smsSent,
          twilio: twilioSent,
          whatsappCloud: waCloudSent
        }
      });
    } catch (err: any) {
      res.status(500).json({ success: false, message: 'Failed to dispatch order status alert', error: err?.message });
    }
  });
  ...
  app.post('/api/orders', (req, res) => {
    ...
    // Delivery Fee: Free above ₹129 AND Free for first order
    ...
    const threshold = zone.freeDeliveryThreshold || 129;
    ...
    const platformFee = 0; // Bazli Platform Fee set to 0 per policy
    const itemsTax = 0;
    const platformTax = 0;
    const tax = 0;
  ...
  app.post('/api/payments/razorpay/create-order', async (req, res) => {
    try {
      ...
      const platformFee = 0; // Bazli Platform Fee (₹0)
    ...
    try {
      ...
      if (apiKey && imageBase64) {
        try {
          ...
          const prompt = `You are Bazli AI Parchhi Vision, a specialized multimodal handwriting OCR engine for Indian grocery lists and store slips.
Your mission is to read even the toughest, messiest, cursive, faded, or ballpoint/pencil handwriting with 99% accuracy.
Language: Can be written in English, Hindi (Devanagari script like १ किलो आलू, २ पैकेट दूध, ५०० ग्राम प्याज, टमाटर, आटा, चीनी, तेल), or Hinglish (e.g. 1kg aaloo, pyaaz, doodh, mirchi, dahi, chakki gehu, amul butter, classmate notebook).

Instructions:
1. Detect each handwritten item line-by-line.
2. Extract the item name clearly in both English/Hinglish and original text.
3. Extract requested quantity and unit (e.g., "1 kg", "500 g", "2 packets", "1 dozen", "2 pcs", "1 litre"). If no quantity is specified, default to "1 unit".
4. Add any handwritten brand names or notes (e.g., "Amul", "Aashirvaad", "Fortune", "Classmate", "Fresh").

Return STRICT JSON:
{
  "detectedTextSummary": "Summary of detected handwritten slip with language and confidence rating",
  "aiEngine": "Gemini 2.5 Flash Multimodal Vision",
  "accuracyRating": "99%",
  "items": [
    {
      "itemText": "Standard item name (e.g. Onion, Potato, Amul Toned Milk, Fresh Paneer, Atta, Maggi Noodles, Classmate Spiral Notebook)",
      "quantity": "quantity requested (e.g. 1 kg, 500g, 2 pkts)",
      "note": "optional brand or special note"
    }
  ]
}`;
          ...
          if (parsed && Array.isArray(parsed.items)) {
            ...
            rawDetectedContent = parsed.detectedTextSummary || 'Handwritten Parchhi Scanned via Gemini 2.5 Flash Vision (99% Accuracy)';
          }
        } catch (geminiErr) {
